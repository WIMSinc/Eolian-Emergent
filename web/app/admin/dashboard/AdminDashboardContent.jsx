...
// Quill touches `document` at import time, so it cannot be evaluated during
// SSR. Loading it dynamically with ssr:false defers it to the browser, which
// is fine here — the editor is behind an authenticated, noindex route.
const ReactQuill = dynamic(() => import("react-quill-new"), {
  ssr: false,
  loading: () => (
    <div className="h-48 border border-zinc-800 bg-[#0A0A0A] animate-pulse" />
  ),
});
...
export default function AdminDashboardContent() {
  ...
  }, [router, fetchPosts, fetchContacts]);
