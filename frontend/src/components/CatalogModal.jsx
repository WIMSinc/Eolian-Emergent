...
export default function CatalogModal({ open, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus("success"); // Still allow download
          ...
          >
            ...
            {status === "success" ? (
              <div className="text-center py-8" data-testid="catalog-success">
                ...
                <button onClick={onClose} className="font-mono text-xs text-zinc-500 hover:text-white transition-colors uppercase tracking-wider">
                  Close
                </button>
              </div>
            ) : (
              <>
                <div className="flex items-center gap-3 mb-2">
                  <Download size={16} className="text-[#FF0B1B]" />
                ...
                <h3 className="font-heading text-xl font-bold text-white uppercase mb-2">
                  ARTAK Product Catalog
                </h3>
                <p className="text-sm text-zinc-400 mb-6">
                  Fill out the form below to download the full ARTAK product catalog.
                </p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-[#FF0B1B] text-white font-mono uppercase text-sm tracking-widest px-8 py-3.5 hover:bg-[#D90412] transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    {status === "sending" ? (
                      <><Loader2 size={14} className="animate-spin" /> Processing...</>
                    ) : (
                      <><Send size={14} /> Download Catalog</>
                    )}
                  </button>
                </form>
              </>
