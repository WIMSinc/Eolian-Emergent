...
export default function Footer() {
  ...
  const handleClick = (e, target) => {
    if (target === "contact" && location.pathname === "/") {
      e.preventDefault();
  ...
  return (
    <footer data-testid="footer" className="border-t border-zinc-800 py-12">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <div>
            ...
            <p className="font-mono text-xs text-zinc-600 tracking-wider">AR & VR Solutions for Defense & Enterprise</p>
          ...
          <div className="flex flex-wrap gap-8">
            {[
              ...
              <Link key={link.label} to={link.to} data-testid={`footer-link-${link.label.toLowerCase()}`} onClick={(e) => handleClick(e, link.label.toLowerCase())} className="font-mono text-sm text-zinc-600 hover:text-white transition-colors tracking-wider uppercase">{link.label}</Link>
            ))}
          </div>
          <div className="text-right">
            <div className="font-mono text-xs text-zinc-700 tracking-wider mb-2">&copy; {new Date().getFullYear()} EolianVR, Inc. All rights reserved.</div>
            <div className="flex gap-4 justify-end">
              <Link to="/terms" data-testid="footer-link-terms" className="font-mono text-[10px] text-zinc-700 hover:text-white transition-colors tracking-wider uppercase">Terms & Conditions</Link>
              <Link to="/privacy" data-testid="footer-link-privacy" className="font-mono text-[10px] text-zinc-700 hover:text-white transition-colors tracking-wider uppercase">Privacy Policy</Link>
            </div>
          </div>
        </div>
      </div>
