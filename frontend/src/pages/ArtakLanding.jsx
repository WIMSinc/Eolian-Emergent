...
const fadeUp = { hidden: { opacity: 0, y: 25 }, visible: (i) => ({ opacity: 1, y: 0, transition: { duration: 0.5, delay: i * 0.08 } }) };
...
export default function ArtakLanding() {
  ...
  return (
    <div data-testid="artak-landing-page">
      ...
      <SEO title="ARTAK - Augmented Reality Team Awareness Kit" description="ARTAK is a multi-domain joint planning and command & control platform." path="/artak" />
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-28">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 md:px-12">
          ...
          <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl font-black uppercase tracking-tighter text-white leading-[0.95] max-w-5xl">Augmented Reality<br /><span className="text-zinc-500">Team Awareness Kit</span></h1>
          <p className="mt-6 text-base md:text-lg text-zinc-400 max-w-3xl leading-relaxed">ARTAK is a multi-domain joint planning and command & control platform. It synchronizes planning and C2 activities across echelons, domains, and warfighting functions by bringing command and staff together into a single digital decision environment.</p>
          <div className="mt-8 flex flex-wrap gap-4">
            <button onClick={() => setCatalogOpen(true)} data-testid="artak-landing-catalog-btn" className="inline-flex items-center gap-2 bg-[#FF0B1B] text-white font-mono uppercase text-sm tracking-widest px-8 py-4 hover:bg-[#D90412] transition-colors"><Download size={16} /> Download Product Catalog</button>
            <Link to="/#contact" className="inline-flex items-center gap-2 bg-transparent border border-zinc-700 text-white font-mono uppercase text-sm tracking-widest px-8 py-4 hover:border-white transition-colors">Contact Us <ArrowRight size={16} /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 border-t border-zinc-800">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12">
          <div className="mb-12">
            <div className="flex items-center gap-3 mb-4"><div className="w-8 h-px bg-[#FF0B1B]" /><span className="font-mono text-xs tracking-[0.2em] text-zinc-500 uppercase">Hardware // Kits</span></div>
            <h2 className="font-heading text-3xl sm:text-4xl font-bold uppercase tracking-tight text-white">ARTAK Kits</h2>
            <p className="mt-4 text-sm text-zinc-400 max-w-2xl leading-relaxed">Deployed in ruggedized cases sealed against moisture and shock. ARTAK kits contain all necessary software, a built-in networking device, an edge-server, a charging-harness, and a rechargeable battery system.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-zinc-800">
            {kits.map((kit, i) => (
              <motion.div key={i} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} data-testid={`landing-kit-${i}`} className="bg-[#0A0A0A] p-6 group hover:bg-[#0D0D0D] transition-colors relative">
                <div className="flex items-center gap-2 mb-3"><Box size={14} className="text-zinc-600 group-hover:text-[#FF0B1B] transition-colors" /><span className="font-mono text-[10px] text-zinc-600 tracking-wider">[KIT.0{i + 1}]</span></div>
          ...
          <div className="mt-12 text-center">
            <div className="flex flex-wrap justify-center gap-4">
              <Link to="/#contact" className="inline-flex items-center gap-2 bg-[#FF0B1B] text-white font-mono uppercase text-sm tracking-widest px-8 py-4 hover:bg-[#D90412] transition-colors">Contact Us to Inquire <ArrowRight size={16} /></Link>
              <button onClick={() => setCatalogOpen(true)} className="inline-flex items-center gap-2 bg-transparent border border-zinc-700 text-white font-mono uppercase text-sm tracking-widest px-8 py-4 hover:border-white transition-colors"><Download size={16} /> Product Catalog</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 border-t border-zinc-800">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12">
          <div className="mb-12">
            <div className="flex items-center gap-4 mb-4"><img src="/artak-logo-white.png" alt="ARTAK" className="h-7 w-auto opacity-70" /><span className="font-mono text-xs tracking-[0.2em] text-zinc-500 uppercase">Storyboards // Field Deployments</span></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-zinc-800">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0} data-testid="landing-case-sage-eagle" className="bg-[#0A0A0A] p-8 group hover:bg-[#0D0D0D] transition-colors relative">
              ...
              <div className="flex flex-wrap gap-2">{["Skydio X2+", "Map Maker", "Meta Quest 3", "GoTENNA", "Starlink", "ATAK"].map((t) => (<span key={t} className="font-mono text-[9px] tracking-wider text-zinc-600 border border-zinc-800 px-2 py-1 uppercase">{t}</span>))}</div>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={1} data-testid="landing-case-nswg8" className="bg-[#0A0A0A] p-8 group hover:bg-[#0D0D0D] transition-colors relative">
              ...
              <div className="flex flex-wrap gap-2">{["GoTENNA", "Starlink", "Live-Fire Tracking", "3D Replay", "AAR"].map((t) => (<span key={t} className="font-mono text-[9px] tracking-wider text-zinc-600 border border-zinc-800 px-2 py-1 uppercase">{t}</span>))}</div>
              <div className="absolute bottom-0 left-0 w-0 h-px bg-[#FF0B1B] group-hover:w-full transition-all duration-700" />
            </motion.div>
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={2} data-testid="landing-case-160soar" className="bg-[#0A0A0A] p-8 group hover:bg-[#0D0D0D] transition-colors relative lg:col-span-2">
              ...
              <div className="flex flex-wrap gap-2 mt-6">{["Skydio X2+", "Map Maker", "AR HMDs", "ROC Drill", "Joint Exercise", "5th SFG(A)", "23rd STS"].map((t) => (<span key={t} className="font-mono text-[9px] tracking-wider text-zinc-600 border border-zinc-800 px-2 py-1 uppercase">{t}</span>))}</div>
              <div className="absolute bottom-0 left-0 w-0 h-px bg-[#FF0B1B] group-hover:w-full transition-all duration-700" />
            </motion.div>
          </div>
        </div>
            ...
            {subPages.map((page, i) => (
              <motion.div key={i} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                <Link to={page.to} data-testid={`artak-nav-card-${i}`} className="block bg-[#0A0A0A] p-6 md:p-8 group hover:bg-[#0D0D0D] transition-colors relative h-full">
                  <div className="flex items-center gap-2 mb-4"><page.icon size={14} className="text-[#FF0B1B]" /><span className="font-mono text-[10px] tracking-[0.2em] text-[#FF0B1B]">{page.label}</span></div>
