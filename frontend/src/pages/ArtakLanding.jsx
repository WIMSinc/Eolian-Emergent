...
import { Shield, Crosshair, Globe, Radio, ArrowRight } from "lucide-react";
import SEO from "@/components/SEO";

const subPages = [
  {
    to: "/artak/national-security",
    icon: Shield,
    label: "[SYS.01]",
    title: "National Security & Public Safety",
    desc: "Use cases for disaster response, search & rescue, security, law enforcement, fire services, and space operations.",
  },
  {
    to: "/artak/disaster-response",
    icon: Crosshair,
    label: "[UC.01]",
    title: "Disaster Response & Emergency Management",
    desc: "Transform fragmented operations into a unified mission picture with real-time drone feeds, IoT data, and geospatial overlays.",
  },
  {
    to: "/artak/search-rescue",
    icon: Globe,
    label: "[UC.02]",
    title: "Search & Rescue / First Responders",
    desc: "Integrate UAV data, terrain models, and team tracking into a shared 3D environment for faster response.",
  },
  {
    to: "/artak/security-protection",
    icon: Shield,
    label: "[UC.03]",
    title: "Security & Protection Services",
    desc: "Fused operational picture combining radar, drone, and sensor data in a single spatial interface.",
  },
  {
    to: "/artak/police-law-enforcement",
    icon: Crosshair,
    label: "[UC.04]",
    title: "Police & Law Enforcement",
    desc: "Shared operational view for urban operations, crowd control, and tactical response.",
  },
  {
    to: "/artak/fire-emergency",
    icon: Radio,
    label: "[UC.05]",
    title: "Fire & Emergency Services",
    desc: "Complete situational awareness — tracking crews, visualizing structures, and modeling wildfire spread in 3D.",
  },
  {
    to: "/artak/space-aerospace",
    icon: Globe,
    label: "[UC.06]",
    title: "Space & Aerospace Operations",
    desc: "Visualize orbital assets, trajectories, and contingencies in immersive 3D for mission command.",
  },
];
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-28">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 md:px-12">
          ...
          <p className="mt-4 text-base text-zinc-400 max-w-3xl leading-relaxed">
            ARTAK software is device agnostic, running across AR, VR, Tablet, Phone, Laptop, and PC. Block 2 is now available with massive improvements in 3D mapping, UI/UX, speed, and multi-domain operations capability.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-zinc-800">
            {subPages.map((page, i) => (
              ...
              >
                <Link
                  to={page.to}
                  data-testid={`artak-nav-card-${i}`}
                  className="block bg-[#0A0A0A] p-6 md:p-8 group hover:bg-[#0D0D0D] transition-colors relative h-full"
                >
                  ...
                  <h3 className="font-heading text-base font-semibold text-white uppercase tracking-wide mb-3 group-hover:text-[#FF0B1B] transition-colors">
                    {page.title}
                  </h3>
                  <p className="text-sm text-zinc-500 leading-relaxed mb-4">{page.desc}</p>
                  <span className="inline-flex items-center gap-2 font-mono text-xs tracking-[0.15em] text-zinc-600 group-hover:text-[#FF0B1B] transition-colors uppercase">
