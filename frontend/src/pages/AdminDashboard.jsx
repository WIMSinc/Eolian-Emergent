...
import {
  FileText, Mail, Plus, Pencil, Trash2, LogOut, Save, X, Eye, Calendar, Tag, Upload, Loader2,
} from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="font-mono text-[10px] tracking-[0.2em] text-zinc-500 uppercase block mb-1.5">Hero Image</label>
          <div className="flex gap-2">
            <input data-testid="post-form-image" value={form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} className={`${inputCls} flex-1`} placeholder="URL or upload..." />
            <label data-testid="hero-image-upload-btn" className="shrink-0 flex items-center gap-2 bg-[#0A0A0A] border border-zinc-800 px-3 py-2.5 text-xs text-zinc-500 hover:text-white hover:border-zinc-600 transition-colors cursor-pointer font-mono uppercase tracking-wider">
          ...
          {form.image && (
            <div className="mt-2 h-20 border border-zinc-800 overflow-hidden">
              <img src={form.image} alt="Preview" className="h-full w-full object-cover opacity-60" />
            </div>
          )}
        </div>
      </div>
